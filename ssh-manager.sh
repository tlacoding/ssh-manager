...
#########################################
# Modified by Thong Le
# Contact at lpmthong.dev@gmail.com
# Github https://github.com/tlacoding/ssh-manager
...
function list_commands() {
  separator
  cecho -ne -cyan "Available commands:"
  echo
...
if [ $# -eq 0 ]; then
  check_sshpass
  separator
  cecho -ne -magenta "List of available servers for $(whoami): "
...
case "$cmd" in
  ...
  cc )
    ...
    if [ $? -eq 0 ]; then
      ...
      cecho -ne -green "connecting to '$alias' ($addr:$port)"
      sshpass -p $pass ssh -o StrictHostKeyChecking=no $user@$addr -p $port
    else
      cecho -ne -red "$0: unknown alias '$alias'"
      exit 1
    fi
    ;;
